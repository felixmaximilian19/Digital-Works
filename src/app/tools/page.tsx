            ...
            <AnimatePresence>
              {filteredTools.map((tool, index) => (
                <motion.div
                  ...
                  className={`relative group glass-card overflow-hidden cursor-pointer border border-white/10 ${tool.featured ? 'ring-2 ring-blue-500/30' : ''}`}
                  onClick={() => window.open(tool.link, '_blank')}
                  whileHover={{ scale: 1.04, rotateX: 6, rotateY: -6, boxShadow: '0 16px 64px 0 rgba(0,0,0,0.25), 0 1.5px 0 0 rgba(255,255,255,0.15)'}}
                >
                  <div className="p-7 flex flex-col gap-3">
                    <div className="flex items-center gap-3 mb-2">
                      {typeof tool.icon === 'string' && tool.icon.length > 0 ? (
                        <span
                          className={`w-10 h-10 rounded-xl flex items-center justify-center text-2xl ${tool.gradient}`}
                          aria-label="Icon"
                          role="img"
                          style={{ fontSize: '2rem', lineHeight: '2.5rem', fontFamily: 'Apple Color Emoji, Segoe UI Emoji, Noto Color Emoji, sans-serif' }}
                        >
                          {tool.icon}
                        </span>
                      ) : (
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-white mb-1 leading-tight">{tool.name}</h3>
